...
<head>
    ...
    <title>Customers - SuperPOS</title>
...
<body>
    ...
    <div class="modal" id="dataModal">
        <div class="modal-content">
            <h2 id="modalTitle" style="margin-bottom: 1.5rem;">Add Customer</h2>
            <form id="dataForm">
    ...
    <script>
        ...
        async function fetchData() {
            ...
            if (data.customers.length === 0) {
                tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;">No customers</td></tr>';
                return;
            }
            data.customers.forEach(item => {
                tbody.innerHTML += `
                    <tr>
                        ...
                        <td>${item.phone || '-'}</td>
                        <td><span style="background: var(--primary-light); color: var(--primary-color); padding: 0.2rem 0.6rem; border-radius: 20px; font-weight: 600; font-size: 0.85rem;">${item.loyalty_points} pts</span></td>
                        <td>
        ...
        document.getElementById('dataForm').addEventListener('submit', async (e) => {
            ...
            const res = await fetch('../api/customers.php', { method: 'POST', body: JSON.stringify(payload) });
            const data = await res.json();
        ...
        async function deleteRecord(id) {
            ...
            const res = await fetch('../api/customers.php', { method: 'POST', body: JSON.stringify({action: 'delete', id: id, csrf_token: csrfToken}) });
            const data = await res.json();
